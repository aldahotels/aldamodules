...
class PurchaseRequest(models.Model):
    ...
    def _check_completed_pr(self):
        ...
        for pr in pr_to_check:
            if all(
                (pl.product_qty - pl.qty_received) == 0
                for pl in pr.line_ids.mapped("purchase_lines").filtered(
...
class PurchaseRequestLine(models.Model):
    ...
    def create(self, values):
        ...
        if portal:
            ...
            # min_qty = product.seller_ids.filtered(
            #   lambda x: x.partner_id.id == request.property_id.seller_id.id
            # ).min_qty
    ...
    def write(self, vals):
        ...
        if portal and product_qty:
            ...
            # min_qty = self.product_id.seller_ids.filtered(
            #   lambda x: x.partner_id.id
            #   == self.request_id.property_id.seller_id.id
            # ).min_qty
            min_qty = min_cost_productinfo.min_qty
            if product_qty < min_qty:
                raise UserError(
            ...
            if self.request_id.review_ids and not no_msg:
                self.request_id.message_post(
                    body=_(
                        "Line edited by {user}: "
                        "<strong>{product} ({old_quantity} -> {quantity})</strong>"
                    ).format(
                        user=self.env.user.name,
        ...
        if lines:
            for hotel in lines.mapped("property_id"):
                ...
                try:
                    ...
                    _logger.exception(
                        _(
                            "Unexpected error creating purchase orders for "
                            "property %(property)s (lines %(lines)s)"
                        )
                        % {
                            "property": hotel.display_name,
                            "lines": filtered_lines.ids,
                        }
                    )
                    continue
